...
Agent::Agent(int ID, std::mt19937 *uniform_random_generator, Environment *env_pointer, double *prior_alpha, double *prior_beta, int N, double *neighbor_weight) : Player(ID, uniform_random_generator, env_pointer, prior_alpha, prior_beta), m_N(N), m_neighbor_weight(neighbor_weight)
{
...
void Agent::action()
{
	...
	for (int k=1; k<=m_K; ++k) {
		std::gamma_distribution<double> gamma_alpha(m_prior_alpha[k-1] + m_N*m_n_hat[k-1], 1.0);
		std::gamma_distribution<double> gamma_beta(m_prior_beta[k-1] + m_N*(m_n_hat[k-1]-m_m_hat[k-1]), 1.0);
