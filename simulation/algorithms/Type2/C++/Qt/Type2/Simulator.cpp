...
Simulator::Simulator(QObject *parent) : QObject(parent)
{
	...
	m_N = json["number of agent"].toInt();
	m_abs_lambda2 = json["abs of lambda2"].toDouble();
	m_delta = 1.0/4.5/m_N;
	m_Tc = m_abs_lambda2>0 ? qCeil(qLn(m_delta/(1.0+m_delta)/qSqrt(m_N))/qLn(m_abs_lambda2)) : 1;
	...
	for (int i=0; i<m_N; ++i) {
		m_agents[i] = new Agent(i+1, m_uniform_random_generator, m_env, m_prior_alpha, m_prior_beta, m_N, m_PerronMatrix[i], m_delta);
...
void Simulator::run()
{
	...
	ts << "[" + tr("environment") + "]: <strong>" << m_envName << "</strong><br/>"
	   ...
	   << "K=<font color=\"red\">" << m_K << "</font>, N=<font color=\"red\">" << m_N << "</font>, T=<font color=\"red\">" << T << "</font>, Tc=<font color=\"red\">" << m_Tc << "</font>, rep=<font color=\"red\">" << repetition_num << "</font>";
	...
	ts << "[" + tr("environment") + "]: " << m_envName << "\n"
	   ...
	   << "K=" << m_K << ", N=" << m_N << ", T=" << T << ", Tc=" << m_Tc << ", rep=" << repetition_num << "\n\n";
	QTextStream(stdout) << msg << flush;
