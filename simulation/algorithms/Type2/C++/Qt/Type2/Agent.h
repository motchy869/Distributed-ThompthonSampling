...
class Agent : public Player
{
	...
	protected:
		...
		double *m_n_hat_dag, *m_m_hat_dag;	//乱数生成器のパラメータ
		double m_delta;	//平均合意アルゴリズムの定数

	public:
		Agent(int ID, std::mt19937 *uniform_random_generator, Environment *env_pointer, double *prior_alpha, double *prior_beta, int N, double *neighbor_weight, double delta);
		~Agent();
		...
		void update_dist_parameter();	//乱数正式のパラメータ更新
};
